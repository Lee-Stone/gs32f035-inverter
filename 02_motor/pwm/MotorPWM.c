...
//Set the Time-Base (TB) Module
	...
	//EPwm1Regs.TBCTL.bit.SYNCOSEL = TB_CTR_ZERO;	//定时器下溢产生同步信号
...
//Set the Trip-zone (TZ) Module
	...
	EPwm2Regs.TZEINT.all = 0x0002;//CBC中断
#else
...
/*******************************************************************************
* Function Name  : PWM输出函数
* Description    : 执行PWM计算/死区补偿/设置PWM/DSP的BUG规避
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
//#ifdef TMS320F2808                  // ePWM模块bug规避, 28035无需规避
	...
	if(EPwm1Regs.TBSTS.bit.CTRDIR == 0)    //下降沿执行的中断
    {
        if(gPWM.U == 0)
    	{
