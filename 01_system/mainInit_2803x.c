/*
 * mainInit_2803x.c → GS32F035P Port (Phase 1)
 *
 * GS32 implementation is ACTIVE. Original TI code is preserved in #if 0 block below.
 * Each function retains the TI API signature for downstream compatibility.
 *
 * TI→GS32 API mapping:
 *   InitSysCtrl()        → Device_init()
 *   InitPll()            → Clock_init() [inside Device_init]
 *   InitFlash()          → Flash_initConfig() [inside Device_init]
 *   InitPeripheralClocks() → Device_enableAllPeripherals()
 *   DisableDog/EnableDog → SysCtl_disableWatchdog / SysCtl_enableWatchdog
 *   KickDog()            → SysCtl_serviceWatchdog
 *   InitPieCtrl()        → Interrupt_initModule()
 *   InitPieVectTable()   → Interrupt_initVectorTable()
 *   InitCpuTimers()      → CPUTimer_init() [in DSP2803x_CpuTimers.c]
 */
...
extern void ConfigCpuTimer(void *Timer, float Freq_MHz, float Period_uSec);
extern void StartCpuTimer0(void);
extern void StartCpuTimer1(void);
...
__attribute__((interrupt)) void rsvd_ISR(void) { gErrorIntCnt++; }

void InitSysCtrl(void)        { Device_init(); DisableDog(); }
void InitInterrupt(void)      { Interrupt_initModule(); Interrupt_initVectorTable(); }
void SetInterruptEnable(void) { /* Phase 2+: Interrupt_enable per IRQ */ }
...
void InitSetGpio(void)  { /* Phase 2: GPIO_setPinConfig + GPIO_setPadConfig */ }
void InitSetAdc(void)   { /* Phase 3: ADC_setupSOC with EPWM trigger */ }
void InitSetPWM(void)   { /* Phase 2: EPWM_setTimeBasePeriod + deadband */ }
void InitSetQEP(void)   { /* Phase 5: EQEP_setPositionCounter */ }
void InitForMotorApp(void)    { /* Phase 6+: motor parameter init */ }
void InitForFunctionApp(void) { /* Phase 8+: function module init */ }
void DisableDog(void) { /* SysCtl_disableWatchdog() */ }
void EnableDog(void)  { /* SysCtl_enableWatchdog() */ }
void KickDog(void)    { /* SysCtl_serviceWatchdog() */ }
void InitPll(uint16_t val)         { (void)val; }
void InitFlash(void)               { }
void InitPeripheralClocks(void)    { }
void InitPieCtrl(void)             { }
void InitPieVectTable(void)        { }

static void (*adc_isr_callback)(void) = (void(*)(void))0;
void ResetADCEndIsr(void)                  { adc_isr_callback = (void(*)(void))0; }
void SetADCEndIsr(void (*p)(void))         { adc_isr_callback = p; }

__attribute__((interrupt)) void ADC_Over_isr(void)   { if (adc_isr_callback) adc_isr_callback(); }
__attribute__((interrupt)) void EPWM1_TZ_isr(void)   { }
__attribute__((interrupt)) void EPWM2_TZ_isr(void)   { }
__attribute__((interrupt)) void SCI_RXD_isr(void)    { }
__attribute__((interrupt)) void SCI_TXD_isr(void)    { }
__attribute__((interrupt)) void EPWM1_zero_isr(void) { }
__attribute__((interrupt)) void PG_Zero_isr(void)    { }

#if 0 // ====== ORIGINAL TI mainInit_2803x.c BELOW (preserved for reference) ======
/***************************************************************************
